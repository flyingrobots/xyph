...
[![License](https://img.shields.io/github/license/xyph)](./LICENSE)
...
- **The Graph is the Plan**: Coordination happens through stigmergy—participants modify the graph, and others observe the changes. The environment *is* the communication medium.
- **Causal Sovereignty**: Every unit of work (Quest) must trace back to a human declaration (Intent). AI agents cannot "hallucinate" work; they must be authorized by the genealogy of intent.
- **Deterministic Convergence**: Built on WARP (Structural Worldline Memory), Xyph ensures that all writers compute the same final state, even after weeks of offline work.
