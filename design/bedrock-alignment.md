...
- git-warp should own worldlines, Strands, observer-relative reads,
  deterministic ticking, transfer/collapse primitives, and bedrock receipts
- XYPH should own policy, governance, cases, briefs, decisions, and the human
  + agent surfaces that govern when bedrock moves are lawful

If XYPH keeps rebuilding those bedrock concepts in application code, it will
quietly become a second graph system sitting on top of git-warp. That is not
the intended architecture.

## Desired Mental Model

The desired bedrock model is:
...
- **Effect emission / delivery observation**: replay-safe bedrock facts for
...
- historical and observer-relative reads are bedrock facts, not XYPH
  inventions
- XYPH may seek a worldline to build product features, but generic time-travel
  debugging remains a bedrock/debugger concern rather than a bespoke XYPH
  shell
- speculative search across many candidate futures is bedrock behavior
...
still application meaning mixed together with bedrock structure. It is useful
...
- Strand mechanics and lifecycle at the bedrock layer
...
- receipts, provenance, and replayable bedrock truth
...
- the law around who, when, and what is allowed to create, braid, abandon, or
  collapse Strands
- doctrine, hills, and operational meaning layered on bedrock facts
...
- backed by a cheap copy-on-write bedrock overlay graph managed by git-warp
- able to tick independently or in lockstep with another worldline when the
  bedrock/runtime allows that behavior
...
like first-class bedrock behavior rather than higher-layer folklore.

At the XYPH product layer, that same capability should be surfaced as
first-class `Strands` rather than as raw bedrock jargon. Humans and agents
should be able to:
...
This should remain bedrock-factual, not governance-rich.

### Stage 3: Transfer / collapse primitive

git-warp should provide the bedrock move that promotes a selected speculative
...
bedrock releases, not local one-off hacks.

The expected flow is:

1. design the bedrock slice in XYPH's corpus first
2. implement and test it in git-warp
3. publish a git-warp release
4. update XYPH to adopt that release
5. verify that one XYPH surface can now rely on the bedrock instead of
   reconstructing it

This is slower than hacking around the gap in XYPH. That slowness is good. It
forces the bedrock boundary to stay honest for every downstream app, not only
for this one.
