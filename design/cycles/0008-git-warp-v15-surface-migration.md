...
Compatibility / bedrock-alignment follow-on

This cycle continues the bedrock-alignment program after
`0004-bedrock-alignment.md`.
...
mixed product and bedrock capabilities together.

`git-warp` `v15` made the split explicit:

- `WarpApp` is the product-facing root
- `WarpCore` is the bedrock/plumbing root
- `working set` became `strand`

That release is the bedrock XYPH is meant to consume. XYPH therefore needs
to stop pretending the old surface still exists.

The migration is not only about compilation:

- XYPH has several services that intentionally perform bedrock work
  (`materialize`, patch provenance, coordinate comparison, strand operations)
- those paths should type against the bedrock root honestly
...
does not fork its own private bedrock dialect or depend on unpublished local
patches.
...
observing one coherent reality after the bedrock upgrade.
...
depending on removed bedrock nouns or silently reading live truth when a
...
- XYPH must consume published bedrock releases, not local one-off patches.
...
- Public `git-warp` noun changes must not leave XYPH speaking the old
  bedrock dialect in active code paths.
...
- retarget XYPH's bedrock-heavy graph root typing to the `WarpCore` surface
- replace removed `working set` bedrock calls/selectors/errors with `strand`
  equivalents
...
### Checkpoint 1: Published bedrock compatibility

1. `npx tsc --noEmit` passes against `@git-stunts/git-warp` `^15.0.1`.
2. XYPH no longer calls removed `working set` bedrock APIs.
...
- This cycle does not treat `WarpCore` as a philosophical failure. XYPH
  already performs genuine bedrock work, so those paths should say so
...
- Keep the migration bounded: adapt the existing architecture to the released
  bedrock split instead of trying to redesign every graph consumer in one
  sweep.
...
3. Did this slice make the bedrock boundary more honest, or did it only
...
- removed `working set` bedrock calls are gone from active XYPH code paths
