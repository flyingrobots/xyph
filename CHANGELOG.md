...
- **Alignment sweep demoted stale planning docs and retired obsolete TUI backlog items** — `TASKS.md` is now a compatibility pointer instead of a live task plan, the README milestone table and older planning docs now explicitly defer to the graph backlog plus the unified design corpus, and an alignment sweep record now captures the rationale. The sweep also retired obsolete backlog items that no longer fit the landing/page model or current product priorities, including `task:tui-suggestion-tab`, `task:dashboard-suggestion-widget`, `task:tui-quest-modal`, and `task:warp-explorer-view`.
