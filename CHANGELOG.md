...
- **`compare_worldlines persist:true` is now explicitly rejected** — the control plane now fails closed when asked to durably record a comparison artifact into `worldline:live`, because doing so would perturb the very live tip being compared and make the tip-vs-tip comparison dishonest.

### Added

- **Preview-only `collapse_worldline` runway** — `xyph api collapse_worldline` now maps onto git-warp’s published coordinate transfer planning surface for canonical derived worldlines. The command currently requires a fresh `comparisonArtifactDigest`, recomputes the source-vs-target comparison to detect drift, supports settlement preview into `worldline:live`, lowers the resulting transfer plan through the shared mutation kernel in dry-run mode, and returns a typed XYPH `collapse-proposal` with per-side observations, sanitized transfer ops, substrate digests, and mutation side-effect preview without mutating live truth
- **Durable `collapse-proposal:*` governance records** — `xyph api collapse_worldline` now accepts `persist: true` to record a preview-only collapse proposal on `worldline:live` using the same `artifactId` it returned in-band. The durable node stores indexed proposal metadata plus a deterministic JSON body containing the preview payload and published git-warp fact exports, making the proposal an attestation target without inventing new substrate serialization
...
- **Read-only `compare_worldlines` preview** — `xyph api compare_worldlines` now maps onto git-warp's published coordinate comparison surface for live-vs-derived and derived-vs-derived reads. The command returns a typed XYPH `comparison-artifact` preview with deterministic artifact identity, per-side observation coordinates, substrate-backed visible patch divergence, visible node/edge/property deltas, and optional target-local comparison detail when `targetId` is provided. It intentionally stops short of attestation or collapse execution
