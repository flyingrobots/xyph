...
- **AION now has an explicit product-design source of truth** — XYPH now defines the human-facing app through [`docs/AION_PRODUCT_DESIGN.md`](docs/AION_PRODUCT_DESIGN.md), a new design document that adapts IBM Design Thinking to AION using sponsor users, hills, playbacks, landing-vs-page rules, a page taxonomy, an attention model, and a capability matrix. README, TUI planning, architecture, and active-direction docs now point at that document instead of competing to define the product independently.
