...
- **Scrollable cockpit panes now show explicit vertical scrollbars** — the worklist and inspector lanes now render a real track with arrows and thumb so operators can see at a glance that more content exists above or below the current viewport instead of discovering scrollability only by trial and error.
