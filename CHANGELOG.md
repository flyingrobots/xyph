...
- **Substrate floor raised to `@git-stunts/git-warp@^14.10.0`** — XYPH now consumes the published substrate release that adds coordinate/working-set visible-state comparison helpers, so worldline comparison preview can rely on published substrate truth rather than local adapters or unreleased behavior.
...
- **Sovereign control-plane foundation** — added `xyph api`, a versioned JSONL machine interface backed by `ControlPlaneService` with `observe`, `history`, `diff`, `explain`, `fork_worldline`, `compare_worldlines`, `apply`, `comment`, `propose`, and `attest` command support. `collapse_worldline`, `query`, and `rewind_worldline` remain reserved without pretending they are implemented yet
