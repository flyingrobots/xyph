...
- **`src/tui/`** — TUI (bijou-tui v3.1): an AION-style cockpit built around one lane rail, one scannable worklist table, one inspector surface, theme presets, and `StylePort`-based styling. The current shell projects `Now`, `Plan`, `Review`, `Settlement`, and `Campaigns` lanes over the same graph-backed snapshot truth.
