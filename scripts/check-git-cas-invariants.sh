...
#
# XYPH bans invoking raw git storage commands (git hash-object, git cat-file, etc.)
# directly. All CAS operations MUST route through @git-stunts/git-cas to guarantee
# Buzhash Content-Defined Chunking (CDC) deduplication and constant-memory streaming.
#
...
if [ $EXIT_CODE -eq 0 ]; then
  echo "✓ No raw git storage CLI violations detected. All CAS operations cleanly encapsulated."
fi
