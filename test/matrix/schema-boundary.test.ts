...
describe("Schema regex boundary — signature.keyId", () => {
  it("valid KEY- format passes", async () => {
    ...
    // Set keyId to KEY- format AFTER resigning to test schema acceptance
    // (signature will be invalid but schema should pass)
    const signed = await resignPatch(patch);
    (signed as any).signature.keyId = "KEY-ABCDEF";
